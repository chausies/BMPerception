...
(function() {
  var AGREE_Q, BLACK_GIF_LENGTH, DATA_CODE, DISAGREE_Q, LEFT, NUM_OF_QUESTIONS, NUM_OF_TASKS, RIGHT, RUN, STRONG_AGREE_Q, STRONG_DISAGREE_Q, Surveys, WALK, WHITE_GIF_LENGTH, get_score, questions, randperm, register_choice, save_survey, wait_for_black_gif, wait_for_white_gif,
  ...
  get_score = function(arr) {
    var l, q, ref, score;
    score = 0;
    for (q = l = 1, ref = NUM_OF_QUESTIONS; 1 <= ref ? l <= ref : l >= ref; q = 1 <= ref ? ++l : --l) {
      switch (arr[q - 1]) {
        case 1:
    ...
    Template.body.helpers({
      ...
      survey_data: function() {
        ...
        Surveys.find().forEach(function(survey) {
          csv += [survey.age, survey.createdAt, survey.part1.join(','), survey.part2.join(','), get_score(survey.part3)].join(',');
          csv += "\n";
        });
        return csv;
      },
